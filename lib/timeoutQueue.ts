...
export class TimeoutQueue<Type> {
  readonly listener = new Listener<"timeout", Type>;
  private fallbackTimeout: number;
  private readonly valueQueue: Type[] = [];
  private readonly timeoutQueue: number[] = [];
  private readonly equalityCheck: (elementA: Type, elementB: Type) => boolean;

  constructor(fallbackTimeout: number = 1000, equalityCheck: (elementA: Type, elementB: Type) => boolean = null) {
    this.equalityCheck = equalityCheck ?? ((a,b) => a === b);
    this.fallbackTimeout = fallbackTimeout;
  }

  add(value: Type, timeout: number=this.fallbackTimeout): this {
  ...
  clear(): void {
    for (const timeoutId of this.timeoutQueue) { clearTimeout(timeoutId); } // stop all timeouts
  ...
  private doAutoDelete(value: Type) {
    this.delete(value);
    this.listener.trigger("timeout", value);
  }

  forEach(callback: (value: Type) => void) {
