import { channelEvents, channelMessage, channelMessageData, channelSendTypes } from "./connBase";
import { Ids } from "./ids";
import { Listener } from "./listener";
import { TimeoutQueue } from "./timeoutQueue";

export class LocalConnection {
  protected readonly clients = new Map<string, LocalClient>();
  protected readonly middleware = new Map<string, (data: channelMessage) => any>()

  buildClient(id: string) {
    if (!this.clients.has(id)) this.clients.set(id, new LocalClient(id, this));
...
export class LocalClient {
  readonly id: string;
  readonly conn: LocalConnection;
  protected readonly channels = new Map<string, LocalChannel>();
  ...
  readonly dmChannel: LocalChannel;
  ...
  buildChannel(id: string) {
    if (!this.channels.has(id)) this.channels.set(id, new LocalChannel(id, this));   
  ...
  static debug_getStructure(client: LocalClient) {
    const id = client.id;
